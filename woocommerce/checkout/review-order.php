...
<div id="order_review" class="clearfix">
	...
	<div id="payment">
						...
						<li class="list-group-item payment_method_<?php echo $gateway->id; ?>">
							...
							<?php
								if ( $gateway->has_fields() || $gateway->get_description() ) :
									echo '<div class="alert alert-info payment_box payment_method_' . $gateway->id . '" ' . ( $gateway->chosen ? '' : 'style="display:none;"' ) . '>';
		...
		<div class="form-row place-order">
			...
			<noscript><?php _e( 'Since your browser does not support JavaScript, or it is disabled, please ensure you click the <em>Update Totals</em> button before placing your order. You may be charged more than the amount stated above if you fail to do so.', 'woocommerce' ); ?><br/><input type="submit" class="btn btn-primary alt" name="woocommerce_checkout_update_totals" value="<?php _e( 'Update totals', 'woocommerce' ); ?>" /></noscript>
			...
			echo apply_filters('woocommerce_order_button_html', '<input type="submit" class="btn btn-primary pull-right alt" name="woocommerce_checkout_place_order" id="place_order" value="' . esc_attr( $order_button_text ) . '" />' );
			?>
