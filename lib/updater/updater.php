 ...
 */
if ( ! function_exists( 'shoestrap_woo_theme_license_form' ) ) {
	function shoestrap_woo_theme_license_form() {
...
add_action( 'shoestrap_updater_form_content', 'shoestrap_woo_theme_license_form' );

/**
 * Register the option
 */
if ( ! function_exists( 'shoestrap_woo_licensing_register_option' ) ) {
	function shoestrap_woo_licensing_register_option() {
		// creates our settings in the options table
		register_setting( 'shoestrap_woo_theme_license', 'shoestrap_woo_theme_license_key', 'shoestrap_woo_theme_license_sanitize' );
	}
	add_action('admin_init', 'shoestrap_woo_licensing_register_option');
}

/*
 * Gets rid of the local license status option when adding a new one
 */
if ( ! function_exists( 'shoestrap_woo_theme_license_sanitize' ) ) {
	function shoestrap_woo_theme_license_sanitize( $new ) {
		$old = get_option( 'shoestrap_woo_theme_license_key' );
		if( $old && $old != $new ) {
			delete_option( 'shoestrap_woo_theme_license_key_status' ); // new license has been entered, so must reactivate
		}
		return $new;
	}
}
 ...
 */
function shoestrap_woo_activate_license() {

	if ( isset( $_POST['shoestrap_woo_theme_license_activate'] ) ) {
		if( ! check_admin_referer( 'shoestrap_woo_nonce', 'shoestrap_woo_nonce' ) ) {
		...
		$api_params = array(
			...
			'license'    => $license,
			'item_name'  => urlencode( 'Shoestrap 3 WooCommerce Child' )
		);
		...
		update_option( 'shoestrap_woo_theme_license_key_status', $license_data->license );

	}

}
add_action( 'admin_init', 'shoestrap_woo_activate_license' );

/*
 * De-activate the license
 */
function shoestrap_woo_deactivate_license() {

	// listen for our activate button to be clicked
	if ( isset( $_POST['shoestrap_woo_theme_license_deactivate'] ) ) {
		...
		if( $license_data->license == 'deactivated' ) {
			delete_option( 'shoestrap_woo_theme_license_key_status' );
		}

	}

}
add_action('admin_init', 'shoestrap_woo_deactivate_license');
